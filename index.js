...
//  Auth + Desktop + Apps
//  Browser uses Galaxy's p.html (real proxy)
// ══════════════════════════════════════
...
// ══════════════════════════════════════
//  BROWSER — loads Galaxy's p.html
//  This is the key change: we iframe p.html
//  which has the real working proxy
// ══════════════════════════════════════
...
function openBrowser(initialUrl) {
  ...
  if (existing) {
    ...
    // If a URL was passed, send it into p.html via postMessage
    if (initialUrl) sendUrlToBrowser(initialUrl);
    return;
  }
  ...
  win.innerHTML = `
    ...
    <div class="window-body" style="padding:0;overflow:hidden">
      <!-- p.html IS the browser — it has tabs, address bar, proxy, everything -->
      <iframe
        id="galaxy-browser-frame"
        src="/p.html"
  ...
  // If a URL was passed, wait for p.html to load then send it
  if (initialUrl) {
    const frame = win.querySelector("#galaxy-browser-frame");
    frame.addEventListener("load", () => sendUrlToBrowser(initialUrl), { once: true });
  }
}

// Send a URL into Galaxy's browser (p.html) via postMessage
// p.html's search.js listens on keyup Enter — we can also just set iframe's
// input value and dispatch an Enter key event via postMessage
function sendUrlToBrowser(url) {
  const frame = document.getElementById("galaxy-browser-frame");
  if (!frame) return;
  try {
    frame.contentWindow.postMessage({ type: "mos-navigate", url }, "*");
  } catch(e) {}
  // Fallback: try direct DOM access if same-origin
  try {
    const input = frame.contentDocument.getElementById("rogoisdabest");
    if (input) {
      input.value = url;
  ...
  win.innerHTML=`
    ...
    <div class="window-body" style="flex-direction:row;overflow:hidden">
      <div class="files-sidebar">
        ...
        <div class="files-sidebar-item active">
  ...
  win.innerHTML=`
    ...
    <div class="window-body">
      <div class="about-body">
        ...
        <div class="about-info">Powered by Galaxy V6 Proxy Engine</div>
      </div>
    </div>`;
  document.getElementById("windows").appendChild(win);
